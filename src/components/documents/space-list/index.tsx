      ...
      <div className={style['document-list']}>
        ...
        <div className="scrollbar h-fit max-h-[80vh] overflow-y-auto">
          <div>
            ...
            <div>{documentsState.documents?.map(item => item.favorite && renderNode(item, item.favorite))}</div>
          </div>
          <div>
            <p className="mt-3 px-3 font-bold">Pages</p>
            <div>{documentsState.documents?.map(item => renderNode(item, item.favorite))}</div>
          </div>
        </div>
      </div>
      {showModalCreate && (
